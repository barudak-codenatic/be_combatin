...
export class ContentProgressService {
    ...
    async createContentProgress(userId : string, moduleId:string, dto : contentProgressDto) {
        try {
            ...
            const completedMaterial = module.materials.filter(m=>m.completed.length > 0).length
            const completedTest = module.test.filter(m=>m.completed.length > 0).length
